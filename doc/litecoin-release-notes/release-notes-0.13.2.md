Titancoin Core version 0.13.2 is now available from:

  <https://download.titancoin.org/titancoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/titancoin-project/titancoin/issues>
...
updates will be released anymore. Without security updates, using a titancoin
...
ECDSA signatures inside Titancoin transactions now use validation using
...
Previous versions of Titancoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Titancoin Core was to raise the default minimum
relay fee.

Titancoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Titancoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running titancoind without having to do any manual configuration.
...
Titancoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Titancoin Core will relay transactions with
...
entirely in Titancoin Core 0.13, and it is at this time undecided whether the
...
Titancoin Core automatically creates a hidden service to listen on, without
manual configuration. Titancoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Titancoin Core is listening, and
...
Titancoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Titancoin
Core to use floating fees. By default, Titancoin Core will use floating
fees.
...
(default: `0.0002` TIC/kB).

At all times, Titancoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) TIC.
Furthermore, Titancoin Core will never create transactions paying less than
...
`titancoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /titancoinrpc>
        ...
        # optional bypass titancoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< titancoinrpc:<password>
    </Location>
...
Restarting titancoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in titancoin.conf
...
titancoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/titancoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Titancoin Core code base has
...
- `titancoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `titancoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Titancoin scripting system that in combination with
...
Titancoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Titancoin Core.
...
The two phases are distinct in the Titancoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Titancoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Titancoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Titancoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of titancoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of titancoins being spent is exactly
...
- **Increased security for multisig:** Titancoin addresses (both P2PKH addresses
  ...
  believed to be provided by Titancoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Titancoin's current ledger (called the UTXO set),
  ...
  note that Titancoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Titancoin users to individually opt-in to almost any change in the Titancoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Titancoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Titancoin
client to Titancoin Core 0.13.2 to signal for segwit support. When a
...
Since Titancoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `titancoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `titancoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
